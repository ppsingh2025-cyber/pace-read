...
const ReaderViewport = memo(function ReaderViewport({
  ...
  /* ── Close popovers when entering focus mode ────────────────── */
  useEffect(() => {
    if (fullHeight) {
      setShowPageJump(false);
  ...
  return (
    ...
    >
      ...
      {/* ── Overlay bar: page nav + word nav — hidden in focus mode ── */}
      {hasWords && !isLoading && !fullHeight && (
      ...
      {/* ── Eye button — always visible when words loaded, focus or not ── */}
      {hasWords && !isLoading && onToggleFocus && (
        <div className={styles.eyeOnlyBar}>
          <button
            className={`${styles.eyeBtn}${fullHeight ? ` ${styles.eyeBtnFocused}` : ''}`}
            onClick={onToggleFocus}
            aria-label={fullHeight ? 'Exit focus mode' : 'Focus on word only'}
            title={fullHeight ? 'Exit focus mode' : 'Focus on word only'}
          >
            {fullHeight ? (
