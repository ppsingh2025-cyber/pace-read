...
const SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: '↺',  color: '#4a6a7a', title: 'Resume reading',       desc: 'Open ☰ → Session Analytics → Past sessions. The 3 most recent file sessions (under 10 MB) and all pasted/URL text sessions resume instantly with a solid "↩ Resume" button — no re-upload needed. Older or larger file entries show "↩ Select file to resume" — re-open the file and your position restores automatically.' },
      { icon: '💾', color: '#4a6a8a', title: 'Auto-save cache',       desc: 'PaceRead saves your last 3 opened files locally (up to 10 MB each) and all pasted or fetched text for instant resume. Opening a 4th file removes the oldest from cache — you\'ll see a notification. Removing a session from history also clears its cache. On iOS when using PaceRead in the browser (not the app), cached sessions may be cleared after extended inactivity — this is an iOS limitation and does not affect the installed app.' },
